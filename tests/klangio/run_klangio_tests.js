/**
 * 🧪 Klangio Test Runner (Moved to /tests/klangio/)
 * KLANGIO: 2025-09-17 - Moved from root directory and updated paths
 * Runs all Klangio integration tests from /tests/klangio/ directory
 */
...
const tests = [
  {
    ...
    command: 'node test_youtube_audio.js', // KLANGIO: 2025-09-17 - Updated path (now in same directory)
    description: 'Tests yt-dlp audio extraction from YouTube'
  },
  {
    name: 'Direct Klangio API',
    command: 'node test_klangio_direct.js', // KLANGIO: 2025-09-17 - Updated path (now in same directory)
    description: 'Tests Klangio API without audio extraction'
  },
  {
    name: 'Klangio with Audio',
    command: 'node test_klangio_with_audio.js', // KLANGIO: 2025-09-17 - Updated path (now in same directory)
    description: 'Tests Klangio API with direct audio URL'
  },
  {
    name: 'Complete Integration',
    command: 'node test_klangio_integration.js', // KLANGIO: 2025-09-17 - Updated path (now in same directory)
