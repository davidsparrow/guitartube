...
export default function WatchLyricsChords() {
  ...
  // Lyrics + Chords Layout: Fixed configuration (no layout switching needed)
  // This page only shows Lyrics (Row 1) + Chords (Row 2), no Tabs (Row 3)
  
  // Control strip states - Lyrics + Chords Layout (Fixed configuration)
  const [showControlStrips, setShowControlStrips] = useState(false)
  const [showRow1, setShowRow1] = useState(true)   // Lyrics (always visible)
  const [showRow2, setShowRow2] = useState(true)   // Chords (always visible)
  // Row 3 (Tabs) not used in this layout
    ...
    <div className="relative h-screen overflow-hidden bg-black">
      ...
      <div className="relative z-10 overflow-hidden px-6 mt-20" style={{
        height: showControlStrips ? `calc(100vh - ${160 +
          (showRow1 ? 51.2 : 0) +
          (showRow2 ? 102.4 : 0)
          // Row 3 (Tabs) not used in Lyrics + Chords layout
        }px)` : 'calc(100vh - 155px)',
      ...
      {showControlStrips && (
        <div className="fixed bottom-16 left-0 right-0 z-40 h-64 bg-transparent px-4 md:px-6">
          ...
          <div className="h-full relative">
            ...
            {/* Row 1: Lyrics Captions - 24% height, positioned from bottom */}
            {showRow1 && (
              <div className="absolute left-0 right-0 flex border-2 border-white rounded-t-lg overflow-hidden h-[24%] transition-all duration-300 bottom-[38%]">
            ...
            {/* Row 2: Chords Captions - 38% height, always at bottom */}
            {showRow2 && (
              <div className="absolute left-0 right-0 flex border-l-2 border-r-2 border-white overflow-hidden h-[38%] transition-all duration-300 bottom-0">
              {/* Left Column - Main Content (96% width) */}
