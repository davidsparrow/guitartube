...
async function testWithNewUser() {
  console.log('🧪 Testing with New User: spasta+free@gmail.com\n');

  try {
    // Step 1: Authenticate with new user
    console.log('🔐 Step 1: Authenticating with new user...');
    const { data: authData, error: authError } = await supabase.auth.signInWithPassword({
      email: 'spasta+free@gmail.com',
      password: 'Free!234'
    });
    ...
    const { data: favorites, error: favoritesError } = await supabase
      .from('favorites')
      .select('id, video_title, video_id')
      .limit(3);
    ...
    if (!favorites || favorites.length === 0) {
      console.log('⚠️  No favorites found. Will use placeholder data for testing.');
    } else {
    ...
    console.log('\n🎉 API Endpoint Testing Complete with Real User!');
